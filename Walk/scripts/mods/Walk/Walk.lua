...
local apply_current_archetype = function()
	local player = Managers.player:local_player_safe(1)
	local profile = player and player:profile()
	current_archetype = profile and profile.archetype and profile.archetype.name or "ogryn"
	walk_speed = mod:get(current_archetype)
end

mod.on_all_mods_loaded = apply_current_archetype
mod:hook_safe(CLASS.GameModeManager, "init", apply_current_archetype)
