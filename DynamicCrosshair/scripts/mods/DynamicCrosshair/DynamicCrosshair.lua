...
local _get_crosshair_position = function(hud_element, dt, ui_renderer, prev_x, prev_y, range, skip_lerp)
    local target_x = 0
    local target_y = 0
    local ui_renderer_scale = ui_renderer.scale
    local player_extensions = hud_crosshair_parent and hud_crosshair_parent:player_extensions()
    local weapon_extension = player_extensions and player_extensions.weapon
    local player_camera = hud_crosshair_parent and hud_crosshair_parent:player_camera()
    ...
    if weapon_extension and player_camera then
        ...
        local pivot_position = hud_element:scenegraph_world_position("pivot", ui_renderer_scale)
...
mod:hook(CLASS.HudElementCrosshair, "_crosshair_position", function(func, self, dt, t, ui_renderer)
    ...
    ghost_crosshair_x, ghost_crosshair_y = _get_crosshair_position(self, dt, ui_renderer, ghost_crosshair_x, ghost_crosshair_y, ghost_range, true)
    local crosshair_x, crosshair_y = _get_crosshair_position(self, dt, ui_renderer, self._crosshair_position_x, self._crosshair_position_y, perform_repositioning and latest_range or MAX_DISTANCE, ghost_crosshair_visible and perform_repositioning and not latest_color_type)
    self._crosshair_position_x = crosshair_x
    self._crosshair_position_y = crosshair_y
