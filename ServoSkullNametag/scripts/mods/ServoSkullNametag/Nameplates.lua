...
local _get_glyph = function(data)
	...
	local color = UISettings.player_slot_colors[player_slot]
		or Color.ui_hud_green_light(255, true)
	return "{#color(" .. color[2] .. "," .. color[3] .. "," .. color[4] .. ")}"
		.. mod:get("icon_" .. data.skull_type) .. "{#reset()}"
end
...
local create_nameplate = function(template_name, in_hub)
	local size = {
		400,
		20,
	}
	...
	local template = {
		...
		create_widget_defintion = function(_, scenegraph_id)
			...
			return UIWidget.create_definition({
				...
				},
				{
					...
					visibility_function = function(content, _)
						return not in_hub and content.wants_visible and content.is_clamped
					end,
				},
				{
		...
		on_exit = function(_, marker)
			mod.untrack_nameplate(marker)
		end,
		update_function = function(_, _, widget, marker, _, dt, _)
			local content = widget.content
			...
			if marker.raycast_initialized then
				...
				line_of_sight_progress = marker.raycast_result and
					math.max(line_of_sight_progress - dt * line_of_sight_speed, 0)
			...
			if marker.draw then
				...
				local clamped_alpha = content.distance > 1 and (255 * (1 - edge_clamp_progress)) or 0
...
local hub_template = create_nameplate("servoskull_nametag_hub", true)
hub_template.max_distance = 15
hub_template.fade_settings = {
	default_fade = 1,
	fade_from = 0,
	fade_to = 1,
	distance_max = hub_template.max_distance,
	distance_min = hub_template.max_distance * 0.5,
	easing_function = math.ease_exp,
}

return {
	servoskull_nametag_mission = create_nameplate("servoskull_nametag_mission", false),
	servoskull_nametag_hub = hub_template,
}
