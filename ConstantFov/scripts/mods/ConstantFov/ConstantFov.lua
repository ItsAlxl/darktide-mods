...
mod:hook(CLASS.Buff, "update_stat_buffs", function(func, self, current_stat_buffs, t)
    func(self, current_stat_buffs, t)
    if not fov_data.allow_vetult then
        local fov_multiplier = current_stat_buffs.fov_multiplier
        if fov_multiplier and VeteranAbilities[self:template_name()] then
